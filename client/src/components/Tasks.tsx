    ...
    return (
        <ul className="space-y-4 max-w-4xl mx-auto">
            {tasks.map((task) => (
                <li
                    key={task.id}
                    className="p-4 border rounded-md border-gray-300 dark:border-gray-700 flex justify-between items-center"
                >
                    <div>
                        ...
                        {task.assigneeId && (
                            <p className="text-sm text-gray-600 dark:text-gray-400">Assignee ID: {task.assigneeId}</p>
                        )}
                    </div>
